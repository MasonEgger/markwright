...
1. A Go/Hugo author who wants DO code labels, environments, line prefixes, and `<^>highlight<^>` in their content, plus the embeds.
2. A Node/unified author who wants to bracket their remark pipeline with DO syntax support.
3. A plain Unix pipeline: `do-md pre < in.md | some-renderer | do-md post > out.html`.
4. A Python consumer who keeps using the extensions in-process and ignores the CLI entirely (unchanged path).

## Design overview

The core idea is that every DO feature decomposes into at most two pure string transforms: a source-stage transform (markdown text in, markdown-plus-HTML text out) and an HTML-stage transform (rendered HTML in, rendered HTML out).
...
| fence | extract directives and flags to `<!-- do-fence:{JSON} -->`, keep the fence | apply label divs, CSS classes, `<ol><li data-prefix>` | the `do-fence` comment |
...
- Fence styling uses a marker comment. The pre stage encodes directives as `<!-- do-fence:{JSON} -->`, which a conformant renderer passes through untouched. The post stage reads the comment and styles the adjacent code block.
...
The command is `do-md`.

```
do-md pre    [--use NAME ...] [--exclude NAME ...]
do-md post   [--use NAME ...] [--exclude NAME ...]
do-md render [--use NAME ...] [--exclude NAME ...]
do-md list
do-md --version
```

- `pre` reads markdown, writes markdown with embedded HTML and marker comments.
- `post` reads HTML, writes HTML with DO styling and scripts applied.
...
2. Preserve HTML comments (needed for the `do-fence` marker). A renderer that drops comments disables fence styling but does not break anything else.
...
- Input with no DO syntax passes through unchanged at every stage.
...
- A renderer that strips HTML comments leaves fence code blocks unstyled; the post stage skips them silently, optionally warning under a `--warn` flag.
- The `\<^>` escape survives both stages and renders as a literal marker, as it does in-process today.
- Malformed or adversarial marker comments are validated before use; a bad JSON payload is skipped, not executed.
...
3. `post` subcommand: highlight, fence apply, signature-based script injection, with tests.
...
- Renderer assumptions (HTML passthrough, comment survival, span-based highlighting) will not hold in every chain. Mitigate with the documented contract, graceful degradation, and an optional `--warn` mode that reports skipped features.
- The refactor could regress tested behavior. Mitigate by keeping adapters thin and running `just check` at each milestone.
- The `do-fence` marker becomes a cross-tool contract. Mitigate by versioning the payload and validating it on read.
...
- Agent-readable output. A static site that serves raw markdown to LLM agents wants the DO syntax resolved into something legible, since `[youtube abc]` and `<^>foo<^>` are presentational noise an agent cannot interpret. This is a clean-text or clean-markdown render target (for example `[youtube abc]` to a plain `Video: <url>` line, `<^>name<^>` to just `name`), not a data format. It is a second serializer over the same structured constructs.
- Structured JSON of the DO constructs, for tools that want to inspect what syntax is present rather than consume HTML. Cheap on the current substrate because the data already exists internally (embed flag dicts, the fence metadata that is already JSON-serialized into its marker).
...
1. Comment survival: the `do-fence` marker rides through the same raw-HTML passthrough the embeds require, so a comment-stripping renderer disables fence styling. For MVP this is accepted (graceful no-op). A data-attribute encoding that survives strict renderers is a possible later enhancement, not MVP.
2. Marker format stability: the `do-fence` payload becomes a cross-tool contract, so it carries a `version` field from day one. The compatibility policy across versions (strict, or best-effort with fallback) is open.
